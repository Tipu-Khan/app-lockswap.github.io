...
const CakeStats = () => {
  ...
  const lockPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = lockPrice.times(circSupply)

  let lockPerBlock = 0
  if (farms && farms[0] && farms[0].lockPerBlock) {
    lockPerBlock = new BigNumber(farms[0].lockPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'Lock Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'LOCK burnt by LockSwaps')}{' '}
          ...
          <LinkExternal href="https://bscscan.com/token/0x13c2d9b130E0830f79B41FDFb158B83875baf7FF?a=0x000000000000000000000000000000000000dead">
            <CardValue fontSize="14px" value={getBalanceNumber(burnedBalance)} decimals={0} />
          </LinkExternal>
        </Row>
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New LOCK/block')}</Text>
